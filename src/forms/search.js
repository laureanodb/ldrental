...
import { TIPOS_INFRACCION, TIPOS_SINIESTRO, STOCK_CATEGORIAS } from '../constants.js';
...
export function doSearch() {
  ...
  const stockCatLabel = k => (STOCK_CATEGORIAS.find(x => x[0] === k) || [0, ''])[1];
  const repuestos = S.repuestos.filter(r => [r.nombre, stockCatLabel(r.categoria), r.notas].join(' ').toLowerCase().includes(q)).slice(0, 8);
  let h = '';
  ...
  if (repuestos.length) h += '<div class="sec-t">Stock de repuestos</div>' + repuestos.map(r => '<div class="card tap" onclick="repuestoForm(\'' + r.id + '\')">' + esc(r.nombre) + ' <span class="small muted">' + (r.stockActual || 0) + ' en stock</span></div>').join('');
