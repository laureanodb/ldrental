...
export function proveedoresView(editId) {
  ...
  h += P.length ? P.map(p => '<div class="card tap" onclick="proveedoresView(\'' + p.id + '\')"><div class="row between"><div>' + esc(p.nombre) + (p.inactivo ? ' ' + badge('mute', 'Inactivo') : '') + '</div>' + (ratingLabel(p.rating) ? badge(ratingCls(p.rating), ratingLabel(p.rating)) : '') + '</div><div class="small muted">' + esc([p.rubro, p.tel].filter(Boolean).join(' · ')) + '</div>' + (p.notas ? '<div class="small muted">' + esc(p.notas) + '</div>' : '') + '</div>').join('') : '<div class="small muted" style="margin-bottom:8px">Todavía no cargaste proveedores.</div>';
...
export async function saveProveedor(editId) {
  ...
  const o = { id: editId || uid(), nombre, rubro: val('pr_rubro'), tel: val('pr_tel'), rating: val('pr_rating'), notas: val('pr_notas'), inactivo: inactivoEl ? inactivoEl.checked : false };
