...
export function proveedoresView() {
  ...
  h += P.length ? P.map(p => '<div class="card row"><div class="grow"><div>' + esc(p.nombre) + '</div><div class="small muted">' + esc([p.rubro, p.tel].filter(Boolean).join(' · ')) + '</div>' + (p.notas ? '<div class="small muted">' + esc(p.notas) + '</div>' : '') + '</div>' + (canDelete() ? '<button class="btn danger sm" onclick="confirmDel(this,()=>delProveedor(\'' + p.id + '\'))">Borrar</button>' : '') + '</div>').join('') : '<div class="small muted" style="margin-bottom:8px">Todavía no cargaste proveedores.</div>';
