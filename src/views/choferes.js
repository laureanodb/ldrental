...
export function viewChoferes() {
  const nInactivos = S.drivers.filter(d => d.inactivo).length;
  return '<h1>Choferes</h1><p class="sub">' + S.drivers.filter(d => !d.inactivo).length + ' en total' + (nInactivos ? ' · <span class="tap" style="text-decoration:underline" onclick="ui.showInactivos=!ui.showInactivos;renderList()">' + (ui.showInactivos ? 'ocultar' : 'ver') + ' ' + nInactivos + ' inactivos</span>' : '') + '</p>' +
...
export function listChoferes() {
  ...
  const L = S.drivers.filter(d => (ui.showInactivos || !d.inactivo) && (!q || [d.nombre, d.dni, d.tel].join(' ').toLowerCase().includes(q))).sort((a, b) => String(a.nombre).localeCompare(String(b.nombre)));
  ...
  return L.map(d => {
    ...
    let b = d.inactivo ? badge('mute', 'Inactivo') : (debt > 0 ? badge('bad', 'Debe ' + money(debt)) : (cars.some(isContract) ? badge('ok', 'Al día') : ''));
